...
void draw_solar_page(double use, double usekwh, double gen, double maxgen, double genkwh, double temp, double mintemp, double maxtemp, double hour, double minute, unsigned long last_emontx, unsigned long last_emonbase)
{
  ...
  if ((millis()-last_emonbase)>300000)
  {
    // small font
    glcd.setFont(font_clR4x6);
  ...
  // freeram
  //glcd.setFont(font_clR4x6);   		//small font
  glcd.drawString_P(1,1,PSTR("RAM= "));
  dtostrf(freeRam(),4,0,str); strcat(str," B");
  glcd.drawString(15,1,str);
...
void draw_power_history_page(int power_history[])
{
  char str[10];
  //GLCD Setup
  glcd.clear;
  glcd.fillRect(0,0,128,64,0);
  //Serial.println("draw_power_history_page initOK");
  glcd.drawLine(0, 58, 127, 58, WHITE);      //x axis
  glcd.drawLine(0, 0, 0, 63, WHITE);      //y axis
  glcd.setFont(font_clR4x6);   		//small font
  glcd.drawString_P(1,59,PSTR("6"));
  glcd.drawString_P(31,59,PSTR("9"));
  glcd.drawString_P(59,59,PSTR("12"));
  glcd.drawString_P(89,59,PSTR("15"));
  glcd.drawString_P(119,59,PSTR("18"));
  
  //Serial.println("draw_power_history_page drawLineOK");
  int i_max_value = getIndexOfMaximumValue(power_history,N_POWER_POINTS);
  //Serial.println("trovato max");
  double max_value = power_history[i_max_value];
  
  dtostrf((max_value),4,0,str); strcat(str," W");
  glcd.drawString(5,1,str);
  
  double scale = 58/max_value;
  int rect_width = 128/N_POWER_POINTS;
  int line_h;
  for (int i=0;i<N_POWER_POINTS;i++) 
  {
    if (power_history[i]>=0) {
      line_h = power_history[i]*scale;
      //Serial.println(line_h);
      glcd.drawRect(i*rect_width, 58-line_h, rect_width, line_h, WHITE);      //
    }
  }
  glcd.refresh();
}
